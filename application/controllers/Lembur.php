...
class Lembur extends CI_Controller {
	...
	public function index($from=0){
		if ($this->session->has_userdata('hasadmin')) {
			$jmlData = $this->ImportLembur->getRowLembur();
			$this->load->library('pagination');
			$config['base_url'] = 'http://hrd.gatrans.net/lembur/index/';
	        ...
	        $config['cur_tag_open'] = '<a href="http://hrd.gatrans.net/lembur/" class="active">';
			...
			$sesiLog = array(
						...
						'uploaded' => $this->ImportLembur->getLembur() //$config['per_page'],$from
					);
