...
class Izin extends CI_Controller {
	...
	public function display($from=0){
		if ($this->session->has_userdata('hasadmin')) {
			$jmlData = $this->DisplayData->showJmlDataIzin();
			$this->load->library('pagination');
			$config['base_url'] = site_url().'/data-izin/display/';
	        ...
	        $config['cur_tag_open'] = '<a href="http://hrd.gatrans.net/data-izin/display/" class="active">';
			...
			$sesiLog = array(
						...
						'data-izin' => $this->DisplayData->showDataIzin($config['per_page'],$from),
